...
export async function POST(request: Request, context: RouteContext) {
	...
	if (!isJsonObject(json.value) || (json.value.action !== "publish" && json.value.action !== "unpublish") || !Array.isArray(json.value.sessionIds) || json.value.sessionIds.some((id) => typeof id !== "string" || id.length > 128)) return NextResponse.json({ ok: false, error: "Expected { action: publish|unpublish, sessionIds: string[] }" }, { status: 400 });
	const env = await getCloudflareEnv();
	if (!env.EVENT_ROOM) return NextResponse.json({ ok: false, error: "EVENT_ROOM binding unavailable" }, { status: 503 });
	const response = await env.EVENT_ROOM.getByName(authorization.access.event.id).fetch("https://event-room/bulk-publication", {
		method: "POST",
		headers: { "content-type": "application/json", "x-ce-event-id": authorization.access.event.id },
		body: JSON.stringify({ action: json.value.action, sessionIds: json.value.sessionIds }),
	});
	let result: unknown;
