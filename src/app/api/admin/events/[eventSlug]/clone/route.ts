...
export async function POST(request: Request, context: RouteContext) {
	...
	const sourceAccess = await authorizeEventAdminApi(db, eventSlug);
	if (!sourceAccess) {
		return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
	}
	...
	if (!schedule.ok) {
		return NextResponse.json({ ok: false, error: schedule.error }, { status: 400 });
	}
	...
	try {
		const created = await cloneEventConfiguration(
			db,
			sourceAccess.event.id,
		...
		return NextResponse.json(
			{ ok: false, error: error instanceof Error ? error.message : "Could not clone event" },
			{ status: 400 },
		);
	}
}
