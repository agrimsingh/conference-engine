...
export default async function EmbedsPage({ params }: Props) {
	...
	const embeds = (await listEmbeds(db, event.id)).map((embed) => ({ ...embed, urls: buildEmbedUrls(origin, event.slug, embed.slug) }));
	return <div className="min-h-dvh bg-neutral-950 text-neutral-200"><AdminEventNav eventSlug={event.slug} /><main className="mx-auto max-w-5xl px-4 py-10"><PageHeader eyebrow="Public widgets" title="Embeds" description="Build event-scoped widgets that always read the approved public program." /><EmbedBuilder eventSlug={event.slug} initialEmbeds={embeds} /></main></div>;
}
