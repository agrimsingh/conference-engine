	...
	return (
		<div className="mt-8 space-y-6">
			<section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
				...
				<p className="mt-1 max-w-2xl text-sm text-neutral-400">Push speakers and accepted (or scheduled/published) sessions into your Accelevents event. Changes here stay the source; Accelevents is updated to match, not the other way around.</p>
			...
			{integration.configured ? <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-neutral-100">Preview and push</h2><p className="mt-1 text-sm text-neutral-400">Preview to see creates, updates, holds, and skips. Push keeps existing Accelevents IDs and skips anything that hasn&apos;t changed since the last sync.</p></div><p className="rounded-md border border-neutral-700 bg-neutral-950 px-2 py-1 text-xs text-neutral-400">Connected: {integration.eventUrl}</p></div><div className="mt-5 flex flex-wrap gap-3"><button type="button" disabled={saving} onClick={() => void runSync(true)} className={buttonClasses("secondary")}>Preview changes</button><button type="button" disabled={saving || !(sync?.dryRun && sync.failures.length === 0)} onClick={() => void runSync(false)} className={buttonClasses("primary")}>Push to Accelevents</button></div>{integration.lastSyncError ? <p className="mt-4 text-sm text-red-300">Last sync failure: {integration.lastSyncError}</p> : null}</section> : null}

			{sync ? <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-5"><h2 className="text-lg font-semibold text-neutral-100">{sync.dryRun ? "Preview" : "Latest push"}: {countActions(sync.actions, "create")} create{countActions(sync.actions, "create") === 1 ? "" : "s"}, {countActions(sync.actions, "update")} update{countActions(sync.actions, "update") === 1 ? "" : "s"}, {countActions(sync.actions, "skip")} unchanged</h2>{sync.failures.length > 0 ? <ul className="mt-4 space-y-2">{sync.failures.map((failure) => <li key={`${failure.kind}-${failure.localId}`} className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-300"><span className="font-medium">{failure.kind} {failure.localId}:</span> {failure.message}</li>)}</ul> : <p className="mt-2 text-sm text-neutral-400">{sync.dryRun ? "Nothing was sent to Accelevents." : "Synced items are linked in Accelevents and will skip next time if unchanged."}</p>}</section> : null}
