...
export function SessionWorkbench({
	...
	return (
		<div className="space-y-8">
			...
			<section>
				<div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-base font-medium text-neutral-100">Sessions</h2><p className="mt-1 text-sm text-neutral-400">Publishing is all-or-nothing: every selected session needs a schedule slot. Publish also approves the current content for the public schedule.</p></div><div className="flex gap-2"><Button variant="primary" onClick={() => void bulk("publish")} disabled={busy || selected.length === 0}>Approve &amp; publish selected</Button><Button onClick={() => void bulk("unpublish")} disabled={busy || selected.length === 0}>Unpublish selected</Button></div></div>
