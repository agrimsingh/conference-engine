...
export default async function FilesPage({ params }: { params: Promise<{ eventSlug: string }> }) {
	...
	return <div className="min-h-dvh bg-neutral-950 text-neutral-200"><AdminEventNav eventSlug={event.slug} /><main className="mx-auto max-w-6xl px-4 py-10"><PageHeader eyebrow="Organizer · Files" title={event.name} description="All speaker deliverables in one place. Download the latest files as a ZIP." />{rows.length ? <FilesLibrary eventSlug={event.slug} rows={rows} /> : <EmptyState title="No uploaded files" description="Create a file request in Deliverables, then uploads appear here with session and speaker metadata." />}</main></div>;
}
