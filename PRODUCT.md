...
- Airtable as an exit, not a second brain: organizers can copy submissions into Airtable (manual or nightly) and keep working in sheets. The programme in this app stays authoritative; Airtable never writes back.
...
- Live production deployment: https://conference-engine.65labs.org. Public playable surfaces: `/demo` launcher into `demo-cfp-to-stage` (interactive read-only CFP form, public schedule/speakers). Writes are blocked by demo mode. Organizer admin, reviewer boards, speaker portal, and admin agent API require creating your own event. Optional local-only writable fixture `aie-sandbox` exists after `npm run db:reset:local` for tests and admin clickthrough (not a production playground).
...
5. **Data stays legible and exportable.** Prefer the public API, CSV/XLSX, and optional Airtable/Accelevents copies over integrations that fork the programme. External tools receive updates from here; they do not become a second source of truth.
