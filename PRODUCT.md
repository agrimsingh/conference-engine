...
- Airtable mirror mode (deliberate judgment, not a missed bidirectional integration): D1 stays the system of record. Organizers push submissions one-way into Airtable via REST (manual push or optional nightly cron opt-in) so the team can keep living in Airtable while the engine keeps the program correct. Never sync Airtable → D1.
...
5. **Data stays legible and exportable.** D1 is the single source of record. Prefer the public API, CSV, and Airtable mirror mode (one-way push) over integrations that fork the truth. Never reverse-sync Airtable into D1.
